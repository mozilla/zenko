...
def show_daily_impressions():
    ...
    if client:
        countries = redshift.get_countries_per_client(
            cache,
            mozilla_tiles=mozilla_tiles,
            client=client,
            campaign=campaign,
            locale=locale)
    else:
    ...
    if client:
        if client == 'Mozilla':
            if campaign:
                # special case since mozilla tiles are allowed to have
                # particular campaigns
                tiles = [x['ids']
                         for x in mozilla_tiles
                         if x['name'] == campaign][0]
                impressions_data = redshift.get_daily_impressions_data(
                    cursor,
                    tile_ids=tiles)
                meta_data = redshift.get_mozilla_meta_data(
                    cache,
                    mozilla_tiles,
            ...
            if country:
                ...
                if locale:
                    ...
                    impressions_data = redshift.get_daily_impressions_data(
                        ...
                        client=client)
            if locale:
                meta_data = redshift.get_client_meta_data(
                    cache,
                    client=client,
                    locale=locale)
            else:
    ...
    if len(impressions_data) == 0:
        error = {
            "message": [
                (
                    "The Redshift Tiles server doesn't seem to be returning" +
                    " any data for this analysis."
                ),
                "Sound odd? Please contact "
            ],
...
def show_country_impressions():
    ...
    if client == 'Mozilla':
        ...
        if campaign:
            tiles = [x['ids']
                     for x in mozilla_tiles
                     if x['name'] == campaign][0]
            countries_impressions_data = (
                redshift.get_countries_impressions_data(
                    cursor,
                    tile_ids=tiles,
        ...
        countries_impressions_data = redshift.get_countries_impressions_data(
            cursor,
            start_date=start_date,
            end_date=end_date,
            client=client,
    ...
    # (including bounds for the slider)
    slider = {}
    if client:
        if locale:
            meta_data = redshift.get_client_meta_data(
    ...
    # now add in the start and end dates if specified, and format all dates
    # according to javascript's style
    if start_date and end_date:
        ...
        slider = {
            'start_bound': "{0}, {1}, {2}".format(
                start_bound.year,
                start_bound.month-1,
                start_bound.day),
            'end_bound': "{0}, {1}, {2}".format(
                end_bound.year,
                end_bound.month-1,
                end_bound.day),
            'start_value': "{0}, {1}, {2}".format(
                start_date.year,
                start_date.month-1,
                start_date.day),
            'end_value': "{0}, {1}, {2}".format(
                end_date.year,
                end_date.month-1,
                end_date.day),
        }
    ...
    return render_template(
        ...
        client=client,
        meta_data=meta_data,
        countries_impressions_data=countries_impressions_data,
        slider=slider,
        specific_tile=specific_tile,
        locale=locale,
        campaign=campaign)
...
def show_locale_impressions():
    ...
    return render_template(
        ...
        tile_id=tile_id,
        specific_tile=specific_tile,
        locale_impressions_data=impressions_data,
        slider=slider,
        campaign=campaign)
...
def show_creative_selection_page():
    ...
    if client == "Mozilla":
        # special case since there are so many different types

        meta = []
        for tile in mozilla_tiles:
            # mozilla_tiles has already been pre-loaded above
            if 'client' not in tile:
                ...
                for x in tile['ids']:
                    tiles.append([
                        ...
                        cache[x]['locale'],
                        cache[x]['created_at']])
                # sort by creative start date, most recent first
                tile['tiles'] = sorted(tiles, key=lambda x: x[4], reverse=True)
                meta.append(
                    {
                        "name": tile['name'],
                        "tiles": len(tile['ids']),
        ...
        return render_template(
            ...
            clients=clients,
            client=client,
            creative=mozilla_tiles,
            mozilla=True,
            mozilla_campaign_meta_data=meta)
        ...
        if locale == "All Locales":
            ...
            tiles = redshift.get_tiles_from_client_in_locale(
                cache,
                client,
                locale)
...
def show_countries():
    ...
    country = request.args.get("country")
    country_impressions_data = redshift.get_country_impressions_data(
        cursor,
        country=country)
    ...
    return render_template(
        "countries.html",
        clients=clients,
        country_impressions_data=country_impressions_data,
        country=country,
        countries=countries)
...
def engagement_testing():
    ...
    impressions_data = redshift.get_daily_impressions_data_for_engagement(
        cursor,
        client=client)
...
def overview():
    ...
    # get the data
    data = {}
    data['impressions'] = redshift.get_overview_data(
        cursor,
        mozilla_tiles,
        cache,
        country=country,
    ...
    # slider parameters
    # must remember that "bound" means the actual box limits and
    # "values" means the pointer positions.
    slider = {  # set defaults
