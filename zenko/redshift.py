...
def build_tiles_cache(cursor):
    """
    Saves a version of the tiles as JSON to the local directory. Only does
    ...
    if redownload:
        print("Refreshing tiles cache from remote server (will take" +
              " ~2 seconds)...")
        # get the tiles
        ...
        # get the countries
        print "done"
        print("Refreshing tile country cache from remote server (could" +
              " take up to ~29 seconds)...")
        cursor.execute("""
            SELECT DISTINCT
                tiles.id,
...
def build_mozilla_tile_list(cache):
    """
    Finds mozilla tiles in the cache. This is complicated because there is
    currently no separate Advertiser/Client table.
    ...
    mozilla_tiles = [
        ...
        },
        {
            ...
            "url_must_match": [
                (
                    "https://www.mozilla.com/firefox/independent/" +
                    "?utm_source=directory-tiles" +
                    "&utm_medium=directory-tiles&utm_campaign=FX10"
                )
            ]
        },
        {
            ...
            'url_must_match': [
                (
                    ...
                    '&referrer=utm_source%3Dmozilla' +
                    '%26utm_medium%3Dbanner' +
                    '%26utm_campaign%3Ddesktop01'
                )
            ]
        },
        {
        ...
        },
        {
            ...
            'url_must_match': [
                (
                    'https://www.mozilla.com/privacy/tips' +
                    '?utm_source=firefox' +
                    '&utm_medium=directorytile' +
                    '&utm_campaign=DPD15'
                )
            ]
        },
        ...
        },
        {
            ...
            "url_must_match": [
                (
                    ...
                    "?utm_source=directory-tiles&utm_medium=firefox-browser"
                ),
                (
    ...
    for x in range(len(mozilla_tiles)):
        if len(mozilla_tiles[x]) > 1:  # must have some sort of rule definition
            ...
            # print "Processing Rule {0} ({1})".format(
            #     x,
            #     mozilla_tiles[x]['name'],
            # )
    ...
    already = set(
        chain.from_iterable([x['ids'] for x in mozilla_tiles if 'ids' in x])
    )

    # print(
    #     "Already categorized {0}/{1} tiles".format(len(already), len(cache))
    # )
    ...
    for tile in mozilla_tiles:
        for other_tile in mozilla_tiles:
            if tile['name'] != other_tile['name']:
                ...
                if len(same) > 0:
                    print "Warning! {0} are in {1} and {2}".format(
                        same,
                        tile['name'],
                        other_tile['name'])
...
def get_tile_meta_data(cache, tile_id):
    """
    Gets the entry for a specific tile in the tiles database and returns it
    as a list of lists
    """
...
def get_mozilla_meta_data(
        cache,
        mozilla_tiles,
        campaign_name=False,
        locale=False):
    """
    Compiles all the tile entries for a campaign in the moz tiles object and
    returns them as a list of lists to be entabulated
    """
    data = {
        'Tile IDs': [],
        'Locales': set(),
...
def get_client_meta_data(cache, client=False, locale=False):
    """
    Compiles all the client tiles entries in the tiles database and returns
    them as a list of lists to be entabulated
    """
...
def get_tiles_from_client_in_locale(cache, client, locale):
    """
    Gets a list of tiles that run in a particular locale for a particular
    client
    """
    tiles = []
    for tile_id, tile in cache.iteritems():
...
def get_client_attributes(cursor, cache, client):
    """
    For a given tile id, gets all possible locales, countries and campaign
    start dates.
...
def get_daily_impressions_data_for_engagement(cursor, client=False):
    """
    Gets aggregated impressions grouped by day for the engagement page,
    pulled from a cache
    """
...
def get_daily_impressions_data(
        cursor,
        tile_id=False,
        client=False,
        country='all',
    ...
    if tile_ids:
        # Compile together results from several tile ids, usually in
        # the same campaign
        if len(tile_ids) == 1:
            ...
            if locale:
                ...
                INNER JOIN countries ON
                    countries.country_code =
                        impression_stats_daily.country_code
                INNER JOIN tiles ON tiles.id = impression_stats_daily.tile_id
    ...
    for day in data:
        ...
        js_date = "Date.UTC({0}, {1}, {2})".format(
            ...
            day[0].day)
        impressions.append(
            [day[0], day[1], day[2], str(ctr)+"%", day[3], day[4], js_date])
    return impressions


def get_countries_impressions_data(
        cursor,
        tile_id=False,
        start_date=False,
        end_date=False,
        ...
        tile_ids=False):
    """
    Gets aggregated impressions grouped by each country. This is for the
    ...
    for day in data:
        day = list(day)
        ctr = round((day[2] / float(day[1])) * 100, 5) if day[1] != 0 else 0
        # why doesn't insert() work
        impressions.append(
            [day[0], day[1], day[2], str(ctr)+"%", day[3], day[4]])
    return impressions


def get_locale_impressions_data(
        cursor,
        client=False,
        start_date=False,
        end_date=False,
...
def get_country_impressions_data(cursor, country=False):
    """
    Gets country impressions data for the 'countries' page. Not to be
    confused with the country-by-country analysis
    """
    ...
    list_of_dates = [
        "Date.UTC({0}, {1}, {2})".format(x[0].year, x[0].month-1, x[0].day)
        for x in data
    ]
...
def get_overview_data(
    ...
    for t in range(len(mozilla_tiles)):
        if 'client' not in mozilla_tiles[t]:  # i.e. can't be sponsored tiles
            # create blank holder for totals per client
            totals = [0 for x in range(len(data[0])-1)]
            ...
            # insert the CTR
            if totals[0] != 0:
                ctr = round((totals[1] / float(totals[0])) * 100, 5)
            else:
                ctr = 0
            totals.insert(2, ctr)
            ...
            mozilla_tiles[t]['created_at'] = "{0}-{1}-{2}".format(
                earliest_created_at.year,
                earliest_created_at.month,
                earliest_created_at.day)

    # now we have to add in all the paid tiles
    clients = set(
        [x for x in get_sponsored_client_list(cache) if x != 'Mozilla']
    )
    ...
    for client in clients:
        ...
        if to_add['stats'][0] != 0:
            ctr = round(
                (to_add['stats'][1] / float(to_add['stats'][0])) * 100,
                5)
        else:
            ctr = 0
        to_add['stats'].insert(2, ctr)
        to_add['client'] = True
        to_add['created_at'] = "{0}-{1}-{2}".format(
            to_add['earliest_created_at'].year,
            to_add['earliest_created_at'].month,
            to_add['earliest_created_at'].day)
